...
export default function DeletedTasksClient() {
  ...
  const { canDelete } = useTeamPermission(user, teamMembers);
  ...
  const handleConfirmPermanentDelete = async () => {
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message || "Failed to permanently delete task",
      );
    }
  };
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredTasks.map((task: Task) => {
              const hasTaskPermission = canDelete(task);
                ...
                >
                  ...
                  <div className="pt-3 border-t border-slate-100 mt-2 flex items-center justify-between">
                    <div className="flex items-center gap-1.5 text-xs text-slate-400">
                      ...
                      <span>
                        {task.dueTo
                          ? new Date(task.dueTo).toLocaleDateString("en-US", {
                              year: "numeric",
                              month: "short",
                              day: "numeric",
                            })
